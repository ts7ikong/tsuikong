...
import cc.mrbird.febs.server.tjdkxm.mapper.OperateMapper;
...
public class OperateServiceImpl extends ServiceImpl<OperateMapper, Operate> implements OperateService {
    ...
    public IPage<Operate> findOperates(QueryRequest request, Operate operate) {
        ...
        queryWrapper.orderByDesc("OPERATE_ID");
        ...
        if (StringUtils.isNotBlank(operate.getOperateUserName())) {
            queryWrapper.and(wapper -> {
                wapper.inSql("OPERATE_PROJECTID", "select PROJECT_ID from p_project where PROJECT_NAME " +
                        "LIKE CONCAT('%','" + operate.getOperateUserName() + "','%')");
                wapper.or().inSql("OPERATE_USERID", "select USER_ID from t_user where REALNAME " +
                        "LIKE CONCAT('%','" + operate.getOperateUserName() + "','%')");
            });
        }
        ...
        if (StringUtils.isNotBlank(operate.getOperateType())) {
            queryWrapper.eq("OPERATE_TYPE", operate.getOperateType());
        }
        ...
        if (projectIds.size() > 0) {
            projects = projectMapper.selectList(new LambdaQueryWrapper<Project>()
                    .select(Project::getProjectName, Project::getProjectId)
    ...
    public void deleteOperate(Operate parcel) {
        this.update(null, new LambdaUpdateWrapper<Operate>().eq(Operate::getOperateId, parcel.getOperateId())
