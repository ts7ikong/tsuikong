...
public class FebsUtil {
    ...
    public static void isProjectNotNull(Long projectId) throws FebsException {
        if (projectId == null) {
            throw new FebsException("请选择项目");
        }
//        Assert.notNull(projectId, "请选择项目");
    }
     ...
     * @return Map<String, Object>
     */
    public static Map<String, Object> getDataTable(MyPage<?> pageInfo) {
        Map<String, Object> data = new HashMap<>(2);
        data.put(PageConstant.ROWS, pageInfo.getMyRecords());
        data.put(PageConstant.TOTAL, pageInfo.getTotal());
        return data;
    }
    ...
    public static String getKey(String loginTypeHeader) {
        String key = String.valueOf(System.currentTimeMillis());
        if (StringUtils.isNotBlank(loginTypeHeader)) {
            //项目端
            if (loginTypeHeader.equalsIgnoreCase(FebsConstant.PC)) {
                key = FebsConstant.PC;
            }
            //app端
            if (loginTypeHeader.equalsIgnoreCase(FebsConstant.APP)) {
                key = FebsConstant.APP;
            }
        }
